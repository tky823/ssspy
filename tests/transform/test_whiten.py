...
def test_whiten(n_channels: int, n_bins: int, n_frames: int):
    ...
    input = real + 1j * imag
    output = whiten(input)  # (n_channels, n_bins, n_frames)

    assert input.shape == output.shape

    covariance = output[:, np.newaxis, :, :] * output[np.newaxis, :, :, :].conj()
    covariance = np.mean(covariance, axis=-1)  # (n_channels, n_channels, n_bins)
    covariance = covariance.transpose(2, 0, 1)  # (n_bins, n_channels, n_channels)
    eye = np.eye(n_channels)  # (n_channels, n_channels)
    eye = np.tile(eye, reps=(n_bins, 1, 1))  # (n_bins, n_channels, n_channels)
